...
def enable_mixed_qat(base_model: GPT) -> dict[str, int]:
    counts = {"attn": 0, "mlp": 0, "bigram": 0}
    for block in base_model.blocks:
        for module in (block.attn.c_q, block.attn.c_k, block.attn.c_v, block.attn.proj):
            module._qat_clip_range = 31
            counts["attn"] += 1
        for module in (block.mlp.fc, block.mlp.proj):
            module._qat_clip_range = 15
            counts["mlp"] += 1
    if base_model.bigram is not None and base_model.bigram.proj is not None:
        base_model.bigram.proj._qat_clip_range = 31
        counts["bigram"] += 1
    return counts
