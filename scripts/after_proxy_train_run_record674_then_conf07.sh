...
LOG_PATH="$PROXY_RECORD674_LOG" run_record674_candidate

if [[ "$RUN_NEARBY_SMOKES" == "1" ]]; then
  LOG_PATH="$PROXY_RECORD674_LAM18_SMOKE_LOG" NGRAM_LAMBDA="0.18" MAX_WINDOWS="128" run_record674_candidate
  LOG_PATH="$PROXY_RECORD674_LAM22_SMOKE_LOG" NGRAM_LAMBDA="0.22" MAX_WINDOWS="128" run_record674_candidate
  LOG_PATH="$PROXY_RECORD674_MIN3_SMOKE_LOG" MIN_COUNT="3" MAX_WINDOWS="128" run_record674_candidate
fi

if [[ "$RUN_CONF07_TAIL" == "1" ]]; then
  LOG_PATH="$PROXY_CONF07_LOG" run_conf07_candidate
fi
