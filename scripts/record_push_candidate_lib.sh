...
record_push_apply_ttt_candidate() {
  ...
  case "$candidate" in
    ...
    ngram674)
      ...
      export NGRAM_EVAL_ORDER="${NGRAM_EVAL_ORDER:-5}"
      export NGRAM_EVAL_ALPHA="${NGRAM_EVAL_ALPHA:-0.20}"
...
record_push_artifact_log_path() {
  ...
  case "$candidate" in
    ...
    ngram659_conf07)
      ...
      ;;
    ngram659_latecool_conf07)
      printf '%s\n' "$log_dir/h200_artifact_ngram_record659_latecool_conf07.txt"
      ;;
    ngram659_latecool_conf07_lamtail)
...
record_push_ngram_log_path() {
  ...
  case "$candidate" in
    ...
    record659_latecool_conf07_smoke)
      printf '%s\n' "$log_dir/h200_artifact_ngram_record659_latecool_conf07_smoke.txt"
      ;;
