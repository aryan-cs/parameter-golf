...
export RUN_ID="${RUN_ID:-h200_ttt_recordstack_80shard_seed${SEED}}"
...
export MAX_WALLCLOCK_SECONDS="${MAX_WALLCLOCK_SECONDS:-$H200_PROXY_TRAIN_LIMIT_SECONDS}"
export VAL_LOSS_EVERY="${VAL_LOSS_EVERY:-500}"
export INITIAL_VALIDATION="${INITIAL_VALIDATION:-0}"
...
export ALLOW_OUT_OF_BUDGET_DEV_RUN="${ALLOW_OUT_OF_BUDGET_DEV_RUN:-0}"

h200_proxy_guard_train_launch "$ITERATIONS" "$MAX_WALLCLOCK_SECONDS" "$ALLOW_OUT_OF_BUDGET_DEV_RUN"

echo "H200 dev proxy budget: <= ${H100_PROXY_REFERENCE_STEPS} steps and <= ${H200_PROXY_TRAIN_LIMIT_SECONDS}s (~${H200_PROXY_TRAIN_LIMIT_MINUTES} min) for a 10-minute 8xH100-equivalent train run" >&2
echo "Launching with ITERATIONS=${ITERATIONS} MAX_WALLCLOCK_SECONDS=${MAX_WALLCLOCK_SECONDS}" >&2
