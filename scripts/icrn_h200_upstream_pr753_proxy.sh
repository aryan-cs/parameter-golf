...
WORKTREE_DIR="${WORKTREE_DIR:-$ROOT_DIR/../parameter-golf-pr753-worktree}"
RECORD_DIR_REL="records/track_10min_16mb/2026-03-25_PodracingII_backoff7gram_8xH100"
...
RUN_ID="${RUN_ID:-h200_upstream_pr753_proxy600${run_suffix}_seed${SEED}}"
PATCH_RUN_DIR="${PATCH_RUN_DIR:-$ROOT_DIR/../parameter-golf-pr753-run-${RUN_ID}}"
LOG_PATH="${LOG_PATH:-$LOG_DIR/${RUN_ID}.txt}"

mkdir -p "$LOG_DIR"

if ! git -C "$ROOT_DIR" show-ref --verify --quiet refs/heads/pr753; then
  git -C "$ROOT_DIR" fetch https://github.com/openai/parameter-golf.git pull/753/head:pr753
fi
if [[ ! -d "$WORKTREE_DIR/.git" && ! -f "$WORKTREE_DIR/.git" ]]; then
  git -C "$ROOT_DIR" worktree add "$WORKTREE_DIR" pr753
fi
