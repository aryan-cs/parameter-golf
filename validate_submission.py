...
def check_train_log(folder: Path) -> tuple[bool, str]:
    ...
    logs = (
        list(folder.rglob("*.log"))
        + list(folder.rglob("*.tsv"))
        + list(folder.rglob("*.txt"))
    )
    if logs:
        names = ", ".join(str(p.relative_to(folder)) for p in logs[:5])
...
def check_submission_json(folder: Path) -> tuple[bool, list[str]]:
    ...
    for canonical, (aliases, expected_type) in REQUIRED_FIELDS.items():
        ...
        if found_key is None:
            if len(aliases) > 1:
                errors.append(
                    f"Missing required field: {canonical} (also accepts: {', '.join(aliases[1:])})"
                )
            else:
